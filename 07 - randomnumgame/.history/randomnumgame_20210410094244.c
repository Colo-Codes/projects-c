...
int main()
{
    ...
    while (attempts < 5 && guessedNumber != randNum)
    {
        ...
        {
            printf("\nYour guess is too low, try again (used %d out of 5 attempts): ", attempts);
        }
        else if (guessedNumber > randNum)
        {
            printf("\nYour guess is too high, try again (used %d out of 5 attempts): ", attempts);
        }
        else
        {
            printf("\nCONGRATULATIONS!!! You guessed correctly in %d attempt(s)! The number was: %d", attempts, randNum);
        }
    ...
    printf("\nGuessed number: %d", guessedNumber);
